...
+ (FSNode *) nodeWithParent:(FSNode *)parent atRelativePath:(NSString *)path {
	return [[FSNode alloc] initWithParent:parent atRelativePath:path];
}

- (id) copyWithZone:(NSZone *)zone {
	return [FSNode nodeWithParent:self.parentNode atRelativePath:self.relativePath];
}
...
- (void) setState:(NSInteger)value {
	...
	if ([checkedItems containsObject:self.parentNode.absolutePath]) {
		...
		for (FSNode *node in self.parentNode.subNodes) {
			if (node == self) {
				if (_state == NSOnState)
